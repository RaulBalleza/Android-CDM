...
public class LessonSevenRenderer implements GLSurfaceView.Renderer {
    ...
    private final ExecutorService mSingleThreadedExecutor2 = Executors.newSingleThreadExecutor();

    /**
     * The current cubes object.
     */
    private Cubes mCubes;
    private ArrayList<Cubes> mCubesArray = new ArrayList<>();
    ...
    //cubeFactor is the number of cubes per Row
    private void generateCubes(int cubeFactor) {
       // mSingleThreadedExecutor.submit(new GenDataRunnable(3, 0));
        mSingleThreadedExecutor.submit(new GenDataRunnable(3, 1));
    ...
    class GenDataRunnable implements Runnable {
        ...
        GenDataRunnable(int requestedCubeFactor, int xCoord) {
                ...
                for (int x = 0; x < 1; x++) {
                    for (int y = 0; y < mRequestedCubeFactor; y++) {
                        for (int z = 0; z < mRequestedCubeFactor; z++) {
                            final float x1 = minPosition + ((positionRange / segments) * (x + xCoord * 2));
                            final float x2 = minPosition + ((positionRange / segments) * ((x + xCoord * 2) + 1));
...
//						Toast.makeText(mLessonSevenActivity, "Out of memory; Dalvik takes a while to clean up the memory. Please try again.\nExternal bytes allocated=" + dalvik.system.VMRuntime.getRuntime().getExternalBytesAllocated(), Toast.LENGTH_LONG).show();								
    ...
    public void onSurfaceCreated(GL10 glUnused, EGLConfig config) {
        ...
        // Load the texture
        mAndroidDataHandle = TextureHelper.loadTexture(mLessonSevenActivity, R.drawable.rubik_texture);
    ...
    public void onDrawFrame(GL10 glUnused) {
        ...
        if (!mCubesArray.isEmpty()) {
            for (Cubes cubes : mCubesArray) {
                if (cubes != null)
                    cubes.render();
            }
        }
